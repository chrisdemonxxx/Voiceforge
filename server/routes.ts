...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/stt", authenticateApiKey, upload.single("audio"), async (req, res) => {
    try {
      ...
      const result = await mlClient.processSTTChunk({
        chunk: audioBase64,
        sequence: 0,
  ...
  app.post("/api/vad", authenticateApiKey, upload.single("audio"), async (req, res) => {
    try {
      ...
      const result = await mlClient.processVAD({
        chunk: audioBase64,
        sequence: 0,
      });
      ...
      res.json({
        segments: result.segments,
        vad_active: result.vad_active,
      });
    } catch (error: any) {
      console.error("[VAD] Error:", error);
      res.status(500).json({ error: error.message || "Failed to detect voice activity" });
    }
  });
